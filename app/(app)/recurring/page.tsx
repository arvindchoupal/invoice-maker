...
export default function RecurringPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card className="overflow-hidden">{items.length ? <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-white/[0.04]"><tr><th className="p-4">Client</th><th className="p-4">Template</th><th className="p-4">Frequency</th><th className="p-4">Next run</th><th className="p-4">Auto send</th><th className="p-4">Status</th></tr></thead><tbody>{items.map((item) => <tr className="border-t border-slate-100 dark:border-white/5" key={item.id}><td className="p-4">{item.client_name || item.client_id}</td><td className="p-4">{item.template_invoice_number || item.template_invoice_id}</td><td className="p-4">{item.frequency}</td><td className="p-4">{String(item.next_run_date).slice(0, 10)}</td><td className="p-4">{item.auto_send ? "Yes" : "No"}</td><td className="p-4">{item.status}</td></tr>)}</tbody></table></div> : <div className="p-5"><EmptyState title="No recurring invoices" body="Create a recurring schedule from an existing invoice template." /></div>}</Card>
    </div>
  );
}
