...
export default function BookkeepingPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card className="overflow-hidden">{expenses.length ? <table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-white/[0.04]"><tr><th className="p-4">Vendor</th><th className="p-4">Category</th><th className="p-4">Date</th><th className="p-4">Total</th><th className="p-4">Status</th></tr></thead><tbody>{expenses.map((expense) => <tr className="border-t border-slate-100 dark:border-white/5" key={expense.id}><td className="p-4">{expense.vendor_name}</td><td className="p-4">{expense.category}</td><td className="p-4">{String(expense.expense_date).slice(0, 10)}</td><td className="p-4">{currency(Number(expense.total), expense.currency)}</td><td className="p-4">{expense.payment_status}</td></tr>)}</tbody></table> : <div className="p-5"><EmptyState title="No expenses yet" body="Add expenses or save AI imports as expenses to generate profit/loss summaries." /></div>}</Card>
    </div>
  );
}
