    ...
    <main className="min-h-screen bg-slate-950 text-white">
          ...
          <div className="grid gap-10 py-10 sm:py-14 lg:grid-cols-[minmax(0,1fr)_minmax(400px,0.82fr)] lg:items-center">
            <div>
              ...
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <Link className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-cyan-300 px-5 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:bg-cyan-200" href={`/free-invoice?template=${page.slug}`}>
                  Create {page.profession.toLowerCase()} invoice <ArrowRight className="h-4 w-4" />
                </Link>
                <Link className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/15 px-5 text-sm font-semibold hover:bg-white/10" href="/tools/gst-calculator">
                  Calculate GST
                </Link>
              </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_0.78fr]">
          ...
          <aside>
            <div className="rounded-[2rem] border border-cyan-300/20 bg-cyan-300/[0.07] p-6 lg:sticky lg:top-6">
              ...
              <Link className="mt-6 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 text-sm font-bold text-slate-950 hover:bg-cyan-200" href={`/free-invoice?template=${page.slug}`}>
                Start free <ArrowRight className="h-4 w-4" />
              </Link>
              <Link className="mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-white/10 px-4 text-sm font-semibold hover:bg-white/[0.05]" href="/tools/business-expense-calculator">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-16 sm:px-6">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {related.map((item) => (
            <Link className="group rounded-3xl border border-white/10 bg-white/[0.04] p-5 transition hover:-translate-y-0.5 hover:border-cyan-300/30" href={invoiceTemplateUrl(item.slug)} key={item.slug}>
